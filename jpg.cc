...
namespace 
{
	...
	void rgb_to_yuv(std::array<std::array<std::uint8_t, 3>, block_size * block_size> &block)
	{
		/* from http://www.fourcc.org/fccyvrgb.php */
		for(auto &[r, g, b] : block)
		{
			std::uint8_t y = std::clamp((0.299 * r) + (0.587 * g) + (0.114 * b), 0.0, 255.0);
			std::uint8_t u = std::clamp(128.0 - (0.168736 * r) - (0.331264 * g) + (0.5 * b), 0.0, 255.0);
			std::uint8_t v = std::clamp(128.0 + (0.5 * r) - (0.418688 * g) - (0.081312 * b), 0.0, 255.0);
			r = y; g = u; b = v;
		}						
	}

	void yuv_to_rgb(std::array<std::array<std::uint8_t, 3>, block_size * block_size> &block)
	{
		/* from http://www.fourcc.org/fccyvrgb.php */
		for(auto &[y, u, v] : block)
		{
			std::uint8_t r = std::clamp(y + 1.402 * (v - 128.0), 0.0, 255.0);
			std::uint8_t g = std::clamp(y - 0.344136 * (u - 128.0) - 0.714136 * (v - 128.0), 0.0, 255.0);
			std::uint8_t b = std::clamp(y + 1.772 * (u - 128.0), 0.0, 255.0);
			y = r; u = g; v = b;
		}						
	}	
